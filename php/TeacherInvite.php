...
if (isset($_GET["id"])) {
    ...
    $adminrs = Database::search("SELECT * FROM `teacher` where `id`='" . $id . "'");
    $an = $adminrs->num_rows;
    ...
    if ($an == 1) {
        ...
        Database::iud("UPDATE `teacher` SET `verification_code`='" . $code . "' WHERE `email`='" . $email["email"] . "'");
        ...
        $mail->isHTML(true);
        $mail->Subject = 'invitations to teachers for registration in the system';
