...
if (isset($_GET["id"])) {
    ...
    $adminrs = Database::search("SELECT * FROM `student` where `id`='" . $id . "'");
    $an = $adminrs->num_rows;
    ...
    if ($an == 1) {
        ...
        Database::iud("UPDATE `student` SET `verification_code`='" . $code . "' WHERE `email`='" . $email["email"] . "'");
        ...
        $mail->isHTML(true);
        $mail->Subject = 'invitations to students  for registration in the system';
